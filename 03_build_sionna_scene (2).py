...
# ---------------------------------------------------------------------------
# POLYGON TRIANGULATION AND 3D EXTRUSION -- turning REAL building footprints
# into REAL 3D shapes, instead of approximating every building as a plain
# bounding-box rectangle.
#
# WHY THIS EXISTS: the earlier version of this script threw away each
# building's true footprint shape and replaced it with the smallest
# rectangle that contains it. That's fast and simple, but it's a real
# source of error -- an L-shaped courtyard building, for instance, would
# be drawn as a solid rectangle, blocking rays through empty space that
# the real building doesn't actually occupy. This section fixes that by
# using the REAL polygon vertices OpenStreetMap already gives us (see
# fetch_osm_buildings() above -- `coords` there is the real outline,
# previously discarded down to a bounding box; now used directly).
#
# No third-party geometry library is used here on purpose: this is a
# from-scratch "ear clipping" triangulation, implemented and verified
# (including on deliberately concave test shapes) specifically for this
# script, so there's no extra dependency to install on top of Sionna
# itself. Ear clipping is a standard, well-understood algorithm for
# exactly this problem (breaking an arbitrary simple polygon -- convex or
# concave, as long as its edges don't cross themselves -- into triangles).
# ---------------------------------------------------------------------------
def polygon_signed_area(poly: np.ndarray) -> float:
    """The shoelace formula. Returns a POSITIVE number if the polygon's
    vertices are listed in counter-clockwise order (as seen from above,
    looking down the -Z axis), and NEGATIVE if clockwise. We use this to
    normalize every building's winding direction before triangulating,
    since OpenStreetMap doesn't guarantee a consistent winding order
...
def _is_convex_vertex(prev, cur, nxt) -> bool:
    """True if the interior angle at `cur` is convex, GIVEN that the
    polygon is wound counter-clockwise. This is a prerequisite check for
    ear-clipping: only a convex vertex can possibly be a valid "ear" to
    clip off next.
    """
...
def _point_in_triangle(p, a, b, c) -> bool:
    """True if point p lies strictly inside triangle a-b-c. Used to check
    that clipping a candidate "ear" wouldn't accidentally cut off some
    OTHER vertex of the polygon that happens to sit inside that triangle
    -- the key correctness check that makes ear-clipping work on concave
    polygons, not just convex ones.
    """
    def sign(p1, p2, p3):
...
def triangulate_polygon(poly_xy: np.ndarray) -> list[tuple[int, int, int]]:
    """Ear-clipping triangulation of a simple polygon (a shape with no
    holes and no self-crossing edges -- true of virtually every real OSM
    building footprint). Input: an Nx2 array of (x, y) vertices in order,
    WITHOUT a repeated closing point (i.e. the first point should NOT
    also appear again at the end). Returns a list of (i, j, k) index
    triples, each identifying one triangle by its three corner indices
    into the ORIGINAL poly_xy array (regardless of any internal winding
    normalization -- the returned indices always refer back to the
    caller's original vertex order).

    If the polygon is degenerate or malformed in a way this simple
    algorithm can't resolve (e.g. genuinely self-intersecting edges from
    bad OSM data), it returns as many triangles as it managed to clip
    before getting stuck, rather than raising an exception -- the caller
    is expected to treat a suspiciously small/empty result as a signal to
    fall back to the bounding-box approximation for that one building.
    """
    ...
    # Ear-clipping's convexity test assumes counter-clockwise winding --
    # flip the vertex order if this particular building's OSM outline
    # happens to be clockwise, but keep track of the original indices so
    # the triangles we return still refer to the CALLER's original order.
    if polygon_signed_area(poly) < 0:
        poly = poly[::-1]
        original_indices = list(range(n))[::-1]
    else:
        original_indices = list(range(n))

    remaining = list(range(n))  # indices into the (possibly reversed) `poly`
    triangles = []
    # A safety guard against an infinite loop on pathological input --
    # real building footprints have at most a few dozen vertices, so this
    ...
    while len(remaining) > 3 and guard < 10000:
        ...
        for pos in range(len(remaining)):
            i_prev = remaining[(pos - 1) % len(remaining)]
            i_cur = remaining[pos]
            i_next = remaining[(pos + 1) % len(remaining)]
            ...
            for other in remaining:
                if other in (i_prev, i_cur, i_next):
                    continue
                if _point_in_triangle(poly[other], a, b, c):
                    is_ear = False
                    break
            if is_ear:
                triangles.append((original_indices[i_prev], original_indices[i_cur], original_indices[i_next]))
                remaining.pop(pos)
                ear_found = True
                break
        if not ear_found:
            # Genuinely stuck (malformed polygon) -- stop rather than loop
            # forever; caller decides whether the partial result is usable.
            break
    if len(remaining) == 3:
        i, j, k = remaining
        triangles.append((original_indices[i], original_indices[j], original_indices[k]))
    return triangles


def build_extruded_building_mesh(footprint_xy: np.ndarray, height_m: float,
                                  vertex_offset: int) -> tuple[list[tuple], list[tuple]]:
    """Turns one real 2D building footprint into a full closed 3D solid --
    a flat roof at the building's real height, a flat floor at ground
    level, and vertical walls following the footprint's true (possibly
    irregular, possibly concave) outline -- instead of a bounding-box
    rectangle. Every face is built with a consistently OUTWARD-pointing
    normal (roof normal points up, floor points down, walls point away
    from the building's interior); this was explicitly verified during
    development against a known test shape before being used here, since
    a backwards normal would make the radio-material physics compute
    reflections as if the surface faced the wrong way.

    `vertex_offset` lets multiple buildings share one combined mesh file
    without their vertex indices colliding -- each building's face
    indices are written relative to where its own vertices start within
    the shared vertex list.

    Returns (vertices, faces): vertices is a list of (x, y, z) tuples;
    faces is a list of (i, j, k) index triples INTO THE COMBINED vertex
    list (i.e. already offset by `vertex_offset`), ready to write
    straight into a Wavefront OBJ file.
    """
    poly = np.asarray(footprint_xy, dtype=np.float64)
    if polygon_signed_area(poly) < 0:
        poly = poly[::-1]
    n = len(poly)

    floor_verts = [(float(x), float(y), 0.0) for x, y in poly]
    roof_verts = [(float(x), float(y), float(height_m)) for x, y in poly]
    vertices = floor_verts + roof_verts  # roof vertex j lives at local index n+j

    roof_tris = triangulate_polygon(poly)

    faces = []
    for i, j, k in roof_tris:
        # Roof: same winding as the (now-CCW) polygon -> normal points +Z (up). Correct.
        faces.append((vertex_offset + n + i, vertex_offset + n + j, vertex_offset + n + k))
    for i, j, k in roof_tris:
        # Floor: REVERSED winding relative to the roof -> normal points -Z (down). Correct.
        faces.append((vertex_offset + i, vertex_offset + k, vertex_offset + j))
    for e in range(n):
        j0, j1 = e, (e + 1) % n
        # Standard CCW-polygon extrusion convention for outward-facing
        # vertical wall normals -- verified during development against a
        # known square test case (see the "why" comment at the top of
        # this section).
        faces.append((vertex_offset + j0, vertex_offset + j1, vertex_offset + n + j1))
        faces.append((vertex_offset + j0, vertex_offset + n + j1, vertex_offset + n + j0))
...
def build_scene_xml(bbox: dict, buildings: list[dict], out_path: Path) -> tuple[Path, list[tuple]]:
    ...
    plane plus every building's REAL extruded 3D shape (see
    build_extruded_building_mesh() above), at the correct real-world
    position and height -- not a bounding-box approximation. All building
    geometry is combined into a single Wavefront OBJ mesh file (written
    alongside the XML) for efficiency, since loading one combined mesh is
    much faster for Mitsuba than loading hundreds of individual shapes.

    If a specific building's footprint can't be triangulated cleanly
    (some real OSM data has minor self-intersections or duplicate
    points), that ONE building falls back to the simpler bounding-box
    approximation rather than being silently dropped from the scene or
    crashing the whole run -- geometry coverage matters more than
    triangulation purity for the handful of buildings this affects.
    """
    building_boxes = []
    obj_vertices = []
    obj_faces = []
    ...
    for i, b in enumerate(buildings):
        ...
        # OSM way outlines repeat their first point at the end to close
        # the ring (see fetch_osm_buildings()'s `refs[0] != refs[-1]`
        # check) -- drop that duplicate closing point before
        # triangulating, since triangulate_polygon() expects an "open"
        # ring (each vertex appearing exactly once).
        footprint = np.column_stack([x, y])[:-1]

        height = b["height_m"]
        try:
            if len(footprint) < 3:
                raise ValueError("fewer than 3 distinct vertices")
            verts, faces = build_extruded_building_mesh(footprint, height, len(obj_vertices))
            if len(faces) == 0:
                raise ValueError("triangulation produced no faces")
            obj_vertices.extend(verts)
            obj_faces.extend(faces)
            n_real_footprint += 1
        except Exception:
            # Fallback: this building's footprint couldn't be triangulated
            # cleanly (rare -- usually a handful of malformed OSM ways per
            # sub-area). Use the bounding-box approximation for just this
            # one building rather than losing it from the scene entirely.
            sx = max((x_max - x_min) / 2.0, 1.0)
            sy = max((y_max - y_min) / 2.0, 1.0)
            cx, cy = (x_min + x_max) / 2.0, (y_min + y_max) / 2.0
            box_footprint = [(cx - sx, cy - sy), (cx + sx, cy - sy),
                              (cx + sx, cy + sy), (cx - sx, cy + sy)]
            verts, faces = build_extruded_building_mesh(np.array(box_footprint),
                                                          height, len(obj_vertices))
            obj_vertices.extend(verts)
            obj_faces.extend(faces)
            n_fallback_box += 1

    print(f"  Geometry: {n_real_footprint} buildings with real extruded "
          f"footprints, {n_fallback_box} fell back to bounding boxes "
          f"(malformed OSM outline)")

    # Write the combined building geometry as a single Wavefront OBJ file.
    # OBJ format is 1-indexed (not 0-indexed like Python), hence the +1
    # on every face index below.
    obj_path = out_path.with_name(out_path.stem + "_buildings.obj")
    obj_lines = [f"v {vx:.4f} {vy:.4f} {vz:.4f}" for vx, vy, vz in obj_vertices]
    obj_lines += [f"f {i+1} {j+1} {k+1}" for i, j, k in obj_faces]
    obj_path.write_text("\n".join(obj_lines), encoding="utf-8")

    # The Mitsuba scene XML now references that OBJ file for all building
    # geometry, plus the same flat ground plane as before.
    lines = [
        '<scene version="3.0.0">',
        ...
        '    <ref id="itu_concrete"/>',
        '  </shape>',
        '  <shape type="obj" id="buildings">',
        f'    <string name="filename" value="{obj_path.name}"/>',
        '    <ref id="itu_concrete"/>',
        '  </shape>',
        '</scene>',
    ]
...
def run_for_operator(op: int, n_sample: int):
    ...
    print(f"  Scene written -> {scene_xml} ({len(building_boxes)} buildings placed, "
          f"see 'Geometry:' line above for the real-footprint vs. fallback-box split)")
