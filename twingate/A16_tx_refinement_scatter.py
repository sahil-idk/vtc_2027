"""
A16_tx_refinement_scatter.py — TX Position Refinement + Diffuse Scattering

Stage 4 ablation: builds on A15 (temporal split, Nelder-Mead position refinement)
and adds ITU TR 38.901 diffuse scattering to the Berlin scene.

The only change vs A15 is three lines after scene load:
  mat.scattering_coefficient = SCATTER_S   # TR 38.901 Table 7.4.2-1 concrete
  mat.scattering_pattern     = LambertianPattern()

With S=0 this reproduces A15 exactly (sanity check).
With S=0.4 this is Stage 4 of Table I.

Usage:
  python A16_tx_refinement_scatter.py [--full] [--device pc1|pc2|pc3|pc4] [--scatter-s 0.4]
  --full        : use ALL training rows during Nelder-Mead
  --device <d>  : device (default: pc1)
  --scatter-s   : ITU S-parameter for itu_concrete (default: 0.4)

Outputs: out/{device}_refinement_results_scatter_s{S}.csv
         out/{device}_refinement_summary_scatter_s{S}.json
...
# --scatter-s argument
_sc_arg = next((sys.argv[i+1] for i, a in enumerate(sys.argv)
                if a == "--scatter-s" and i+1 < len(sys.argv)), "0.4")

# ── Constants ────────────────────────────────────────────────────────────────
DEVICE    = _dev_arg
SCATTER_S = float(_sc_arg)          # TR 38.901 S-param: 0.0=none, 0.4=concrete default
...
# Output suffix encodes S so runs with different S values don't overwrite each other
S_TAG = f"s{SCATTER_S:.2f}".replace(".", "p")   # e.g. s0p40
CKPT_PATH    = OUT / f"{DEVICE}_refinement_results_scatter_{S_TAG}.csv"
SUMMARY_PATH = OUT / f"{DEVICE}_refinement_summary_scatter_{S_TAG}.json"
...
def main():
    # ── Load data ─────────────────────────────────────────────────────────────
    print("=" * 65)
    print(f"A16  —  TX Refinement + Diffuse Scattering  ({DEVICE}, S={SCATTER_S:.2f})")
    print("=" * 65)
    ...
    # ── Load Sionna scene + enable diffuse scattering ─────────────────────────
    ...
    # TR 38.901 Table 7.4.2-1: concrete S-parameter ~0.4 at 2 GHz, Lambertian pattern
    mat = scene.radio_materials["itu_concrete"]
    mat.scattering_coefficient = SCATTER_S
    mat.scattering_pattern     = LambertianPattern()

    solver = PathSolver()
    print(f"  Scene loaded. itu_concrete: S={SCATTER_S:.2f}, "
    ...
    for _, tower in wcl.iterrows():
        ...
        try:
            ...
            # Full Sionna on train (capped) + ALL val rows
    ...
    print(f"\n{'='*65}")
    print(f"FINAL RESULTS  —  {DEVICE}  |  S={SCATTER_S:.2f}  |  {len(all_results)} towers")
    print(f"{'='*65}")
    ...
    print(f"\nSaved: {CKPT_PATH.name}  {SUMMARY_PATH.name}")
