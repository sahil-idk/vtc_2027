...
Extends A15 from 2D (Δx, Δy) to 4D (Δx, Δy, height, azimuth):

  WCL init
    → Coarse azimuth grid (8 directions × fixed height) to seed azimuth
    → Nelder-Mead 4D: (Δx, Δy, Δheight, azimuth_deg)
        → Sionna RT with tr38901 sector antenna
        → OLS calibration on ALL training rows → train MAE
    → Refined (lat, lon, height, azimuth)
  → Final Sionna + OLS on ALL train + ALL val rows
  → Val MAE

Why tr38901:
  Each PCell_Cell_Identity IS a sector (one lobe of a 3-sector tower).
  A sector antenna has ~65° 3dB horizontal beamwidth and ~8 dBi gain in the
  main lobe. Isotropic (A15) cannot model this — tr38901 can.
...
Expected runtime: ~8-30 min/tower depending on n_train. pc1 ≈ 12-24h total.
...
N_OPT_ROWS     = 40      # rows per NM eval (same GPU batch as A15 — keeps NM fast)
...
print("=" * 65)
print("A16  —  4D TX+Antenna Refinement  (pc1, tr38901 + Nelder-Mead)")
print("=" * 65)
print(f"  ALL training rows used (N_OPT_ROWS=None, MAX_EVAL_TRAIN=None)")
print(f"  Antenna: tr38901 sector, tilt={TX_TILT_DEG}° fixed, height+azimuth optimized")
print(f"  Search: ±{SEARCH_RADIUS:.0f}m XY | height [{HEIGHT_MIN},{HEIGHT_MAX}]m | azimuth 0-360°")
...
def compass_to_sionna_yaw(azimuth_deg):
    ...
    North bearing 0° → +y → yaw = π/2.
    East  bearing 90° → +x → yaw = 0.
    """
...
# ── Sionna batch runner (now includes height and azimuth) ─────────────────────
_call_counter = [0]
...
def main():
    ...
    # ── Load Sionna scene with tr38901 sector antenna on TX ───────────────────
    ...
    # ── Checkpoint ────────────────────────────────────────────────────────────
    ckpt_path = OUT / "pc1_a16_results.csv"
    if ckpt_path.exists():
    ...
    for _, tower in wcl.iterrows():
        ...
        try:
            ...
            # ── WCL baseline at initial height + az=0 (isotropic replaced by tr38901) ──
            ...
            # ── Coarse azimuth grid: find best initial direction ───────────────
            ...
            for az in AZIMUTH_GRID_DEG:
                ...
                if mae < best_az_mae:
                    best_az_mae = mae
                    best_az     = float(az)
            print(f" best={best_az:.0f}°  mae={best_az_mae:.3f} dB", flush=True)

            # ── Nelder-Mead 4D: [dx, dy, dh, azimuth_deg] ────────────────────
            ...
            # ── Final eval: ALL train rows + ALL val rows, WCL vs refined ─────
    ...
    print(f"\n{'='*65}")
    print(f"FINAL RESULTS  —  {DEVICE} A16  |  {len(all_results)} towers")
    print(f"{'='*65}")
    ...
    with open(OUT / "pc1_a16_summary.json", "w") as f:
        json.dump(summary, f, indent=2)
    print(f"Saved: out/pc1_a16_results.csv  out/pc1_a16_summary.json")
