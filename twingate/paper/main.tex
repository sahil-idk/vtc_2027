...
dead-zone completeness of $C=0.80$ and reduces RSRP error to 3.02\,dB
and 4.04\,dB across the two operators (well within the
3GPP~TS~36.133 accuracy bound). It also independently flags the same
dead zone with F1\,$=$\,0.77 and recovers 86.5\,\% of a measured
30.4\,dB coverage collapse purely from ray-traced geometry, showing
that DT readiness can be established directly from drive-test data,
with no ground-truth infrastructure needed.
\end{abstract}
...
sources, and city-scale calibration~\cite{beyraghi2025ris} shows that,
given known base-station coordinates, optimizing material properties
alone can substantially narrow the gap between simulated and
region-averaged measured coverage. That result, however, assumes known
tower positions---a luxury public V2X datasets do not have. Recovering
...
laps and physics-based ray tracing from map geometry alone, converge on
the same physical anomaly without either ever seeing the other's
...
\begin{enumerate}
  ...
  \item Development of an inverse ray-tracing localization pipeline
        ...
        calibrated physics-based ray-tracing model incorporating
        standards-derived material properties and diffuse scattering,
        achieving RSRP accuracy (M1) well within accepted digital-twin
        and 3GPP measurement-accuracy bounds on the held-out day.
  \item Demonstration of twin-gate convergence, a cross-validation
        ...
        tower coordinates---the strongest single piece of evidence for
        the claim above.
\end{enumerate}
...
Section~\ref{sec:intro} map onto this architecture as follows: Gate~1
alone produces the measurement-quality metric $C$; Gate~2 alone
produces the geometric-fidelity metric M1; and the
...
locations Gate~1 identifies---already a first, narrower instance of the
twin-gate cross-checking developed further in
...
We emphasize that MAE is the optimization objective, not the pipeline's
actual target: the target is a geometrically and materially faithful
digital twin, and MAE minimization is its practical proxy, since RSRP is
the only measurement available to constrain candidate geometry when
operator topology is withheld, and a physically correct geometry is
expected to predict real RSRP more accurately than an incorrect one
under a reasonably faithful forward model. This proxy relationship is
not guaranteed to be one-to-one: the objective is multi-modal in
azimuth, so an MAE improvement is necessary but not sufficient evidence
of correct geometric recovery on its own. Section~\ref{sec:gate2results}'s
held-out-day discipline and the blind dead-zone prediction in
Section~\ref{sec:discussion} are what supply the additional evidence
that recovered geometry, not merely training fit, is responsible for
...
count. Rather than a binary pass/fail gate, M1 is tracked as a
quantitative indicator of how faithfully the reconstructed digital
twin, comprising recovered tower geometry, standards-derived material
properties, and diffuse scattering, reproduces the real propagation
environment: a physically accurate twin should predict real RSRP more
closely than an inaccurate one, so M1 is expected to fall as each stage
recovers more of the true geometry (Section~\ref{sec:gate2results}).
3GPP TS~36.133 specifies absolute RSRP measurement accuracy of
$\pm$6\,dB under normal conditions~\cite{3gpp36133}; error below this
bound is statistically indistinguishable from the ground-truth label's
own measurement uncertainty. Comparable calibrated ray-tracing digital
twin platforms report similar-magnitude error against real-world
validation (4.99\,dB path-loss RMSE after calibration, versus 8.11\,dB
uncalibrated) and treat power errors below 6\,dB, achieved via a bounded
ray-interaction order, as an acceptable engineering target for
network-planning-class digital twin tasks~\cite{yu2025mart6g}.
...
$C$ rises monotonically with $d_\text{recur}$: 0.00, 0.50, 0.80, 0.875,
and 1.00 at 50, 75, 100, 125, and 150\,m respectively. This is expected,
not concerning: a larger radius can only make cross-vehicle corroboration
easier, so $C\to1$ as $d_\text{recur}$ grows reflects the metric
saturating, not the DT being validated. We fix $d_\text{recur}=100$\,m
\emph{a priori} from GPS centroid uncertainty at 1\,Hz sampling
(Section~\ref{sec:gate1}), well short of this saturation point, rather
than selecting the value that maximizes $C$.
...
each stage as an indicator of how much of the true tower geometry and
material behavior each stage recovers; all MAEs are weighted by
per-tower validation-row count.

\begin{table*}[!t]
  \centering
  \caption{Gate~2 Calibration Pipeline: RSRP MAE (M1) on Held-Out Day~3 (June~24),
           tracking how progressively recovered tower geometry, orientation,
           and material fidelity (Stages~0--4) reduce prediction error toward
           the 3GPP~TS~36.133 absolute RSRP accuracy bound of $\pm$6\,dB~\cite{3gpp36133}.
  ...
  \begin{tabular}{lccc}
    \toprule
    Pipeline Stage & Description & Op.~1 MAE (dB) & Op.~2 MAE (dB) \\
    \midrule
    Stage~0 & Per-cell mean, no ray tracing                          & 4.40 & 5.48  \\
    Stage~1 & Sionna RT + OLS gain, crowd-sourced TX positions       & 3.61 & 5.48  \\
    Stage~2 & + 2D Nelder-Mead position opt.\ (isotropic antenna)   & 2.78 & 4.48  \\
    Stage~3 & + azimuth \& height opt.\ (4D, TR~38.901 sector)
      & 3.03$^{\ddagger}$ & 4.07$^{\S}$ \\
    \textbf{Stage~4} & \textbf{+ ITU diffuse scattering ($S=0.4$, Lambertian)}
      & \textbf{3.02}$^{\ddagger}$ & \textbf{4.04}$^{\S}$ \\
    \midrule
...
spanning the full compass range and heights that are not clamped to the
search-space boundary: a sanity check against a degenerate optimizer
artifact, corroborating but not independently proving that the
recovered geometry is physically real. Under this check, Stage~3
reaches 3.03\,dB. Operator~2 (104 towers) reaches 4.07\,dB at
...
The strongest evidence that these stage-by-stage reductions reflect
recovered physical structure, rather than training-set curve-fitting, is
not in this table at all: Section~\ref{sec:convergence_result} and
Section~\ref{sec:discussion} report a blind, location-specific
prediction in which Gate~2's optimized geometry, fit entirely on rows
excluded from a 150\,m radius around a location it was never told
about, predicts a 26.3\,dB signal drop there against a measured
30.4\,dB collapse. Unlike the aggregate MAE figures above, that is a
structural, out-of-sample prediction a pure fit to average error would
have no particular reason to reproduce; the M1 figures in this table
are best read as a supporting, necessary-but-not-sufficient statistic
for that stronger evidence, not as the primary fidelity evidence
themselves.

\textbf{Overfitting check.} Each stage adds free parameters (height,
azimuth, scattering), which raises the question of whether later stages
merely fit training-day noise rather than recovering real geometry.
The evaluation protocol above guards against this directly: parameters
are optimized only against training-day MAE and evaluated exactly once
on day~3 with no further tuning, so a stage that mainly fit training
noise would show flat or worsening held-out error, not the improving
error every stage actually shows. The monotonic held-out improvement in
Table~\ref{tab:gate2} is therefore itself evidence that the added
geometric freedom captures real structure rather than training noise.

\textbf{M1 accuracy in context.}
...
can reproduce. Stage~4's M1 of 3.02\,dB (Operator~1) and 4.04\,dB
(Operator~2) both fall comfortably within the 6\,dB instrument bound and
at or below the channel's own natural shadow-fading variability: the
residual error is consistent with irreducible measurement and channel
randomness rather than a geometric or material shortfall in the
reconstructed twin.
...
correction can recover. This rules out the calibration parameters alone
explaining the achieved accuracy, though it does not on its own establish
that the specific recovered geometry is correct rather than one of
several candidate geometries fitting training data comparably well; the
blind, location-specific prediction described next is the evidence that
does. Second, a calibration study has no reason to
...
above random), and geometric fidelity within accepted measurement-accuracy
and digital-twin engineering bounds for both operators
(M1\,$=$\,3.02/4.04\,dB).
