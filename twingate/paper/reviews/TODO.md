...
- [x] ✅ **C2 — "each validated on a held-out temporal split" overclaims** *(fixed)*
  ...
  **Applied fix (Option B):** "each validated on a held-out temporal
  split" → "each evaluated on a held-out temporal split," matching the
  paper's own already-established wording for this exact three-metric
  claim elsewhere (§IV-B, Conclusion: "TWINGATE evaluates all three
  readiness criteria..."). Conclusion's separate "validated end-to-end"
